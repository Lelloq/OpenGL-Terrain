...
Plane::Plane()
{
	//Converts the plane vertices with data that includes tangents and bitangents
	normalMapper newVerts = normalMapper(planeVertices, 32, 8);
	newVerts.calcTB(planeIndices, 6);
	newVerts.Unpack(14);
	...
	m_EBO = new EBO(1, 6, planeIndices);
	m_Dif = new Texture();
	m_Spec = new Texture();
	m_Norm = new Texture();
	m_Parallax = new Texture();

	//Load textures
	m_Dif->loadTexture("images/pavementBrick/diffuse.jpg");
	m_Spec->loadTexture("images/pavementBrick/roughness.jpg");
	m_Norm->loadTexture("images/pavementBrick/normal.jpg");
	m_Parallax->loadTexture("images/pavementBrick/displacement.png");
